...
export class EmailController {
  static async quickSync(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const result = await EmailSyncService.initiateSync(
        req.user.userId,
        syncStrategy as any
      );
      res.json(
        createSuccessResponse(
          result,
          "Quick sync initiated - recent emails loading"
        )
      );
    } catch (error) {
  ...
  static async fullSync(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      res.json(
        createSuccessResponse(
          result,
          "Full sync initiated - importing all emails in background"
        )
      );
    } catch (error) {
    ...
    try {
      ...
      res
        ...
        .json(
          createErrorResponse(
            ERROR_CODES.INTERNAL_ERROR,
            "Failed to update email category"
          )
        );
    }
  }
  ...
  static async getCategories(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const categories = await EmailCategoryService.getUserCategories(
        req.user.userId
      );
  ...
  static async getEmailsByCategory(req: AuthenticatedRequest, res: Response) {
    try {
      if (!req.user) {
        return res
          .status(401)
      ...
      res.json(
        createSuccessResponse(result, `Emails in category: ${category}`)
      );
    } catch (error) {
      ...
      res
        ...
        .json(
          createErrorResponse(
            ERROR_CODES.INTERNAL_ERROR,
            "Failed to fetch emails by category"
          )
        );
    }
  }
  ...
  private static handleSyncError(error: unknown, res: Response) {
    if (error instanceof Error) {
      if (error.message === "Sync already in progress for this user") {
        return res
          .status(409)
          .json(createErrorResponse(ERROR_CODES.INTERNAL_ERROR, error.message));
      }
